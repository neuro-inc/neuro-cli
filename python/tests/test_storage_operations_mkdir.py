...
class TestNormalCases:
    def test_fix_leading_platform_slash(self, alice_mkdir, partial_mocked_store):
    ...
    def test_with_principal_bob_file(self, alice_mkdir, partial_mocked_store):
    ...
    def test_with_principal_file_ensure_slash(self, alice_mkdir, partial_mocked_store):
        alice_mkdir.mkdir("storage://alice/data/foo.txt/", partial_mocked_store)
        partial_mocked_store().mkdirs.assert_called_once()
        partial_mocked_store().mkdirs.assert_called_with(path="/alice/data/foo.txt")
...
@patch("neuromation.cli.rc.Config.get_platform_user_name", new="alice")
def test_mkdir_alice_omitted_user_no_leading_slash(alice_mkdir, http_backed_storage):
    alice_mkdir.mkdir("storage:foo", http_backed_storage)
    aiohttp.ClientSession.request.assert_called_with(
        method="PUT",
        json=None,

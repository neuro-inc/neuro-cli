...
platform_tree = [
    {
        'path': '/',
        'files': [FileStatus("/alice", 0, "DIRECTORY"),
                  FileStatus("/bob", 0, "DIRECTORY")]
    },
    {
        'path': '/alice',
        'files': [FileStatus("platform_existing", 0, "DIRECTORY")]
    },
    {
    ...
    {
        'path': '/alice/platform_existing/di1',
        'files': []
    },
    {
    ...
    {
        'path': '/bob',
        'files': [FileStatus("bob_data", 0, "DIRECTORY")]
    },
    {
        'path': '/bob/bob_data',
...
class TestCopyRecursiveLocalToPlatform:
    ...
    def test_ok(self,
        ...
        op = CopyOperation.create('alice', 'file', 'storage', True)
        op.copy_file = mock
        op.copy(urlparse('file:///'),
                urlparse('storage:///platform_existing/'),
                partial_mocked_store)

        assert mock.call_count == 2
        mock.assert_any_call('/dry.exe',
                             '/alice/platform_existing/dry.exe',
                             partial_mocked_store)
        mock.assert_any_call('/localdir/abc.txt',
                             '/alice/platform_existing/localdir/abc.txt',
                             partial_mocked_store)

    def test_ok_copy_bob_data(self,
                              mocked_store, partial_mocked_store, monkeypatch):
        self._structure(mocked_store, monkeypatch)
        transfer_mock = Mock()
        ...
        op.copy(urlparse('file:///localdir/'),
                urlparse('storage://bob/'), partial_mocked_store)
    ...
    def test_ok_copy_into_root_data(self,
        ...
        op.copy_file = transfer_mock
        op.copy(urlparse('file:///localdir/'),
                urlparse('storage:///'), partial_mocked_store)
    ...
    def test_source_doesnot_exists(self,
        ...
        with pytest.raises(ValueError, match=r'Source should exist'):
            op.copy(urlparse('file:///non_existing/'),
                    urlparse('storage:///'),
                    partial_mocked_store)
...
class TestCopyNonRecursivePlatformToLocal:
    ...
    def test_source_is_dir(self,
        ...
        op = CopyOperation.create('alice', 'file', 'storage', False)
        op.copy_file = transfer_mock
        with pytest.raises(ValueError, match=r'Source should be file.'):
            op.copy(urlparse('file:///localdir/'),
                    urlparse('storage:///platform_existing/'),
                    partial_mocked_store)
    ...
    def test_source_file_target_dir(self,
                                    mocked_store,
    ...
    def test_source_file_target_root_trailing_slash(self,
                                                    mocked_store,
                                                    partial_mocked_store,
                                                    monkeypatch):
        self._structure(mocked_store, monkeypatch)
    ...
    def test_source_file_target_root_no_trailing_slash(self,
                                                       mocked_store,
                                                       partial_mocked_store,
                                                       monkeypatch):
        self._structure(mocked_store, monkeypatch)
    ...
    def test_source_file_target_empty(self,
                                      mocked_store,
                                      partial_mocked_store, monkeypatch):
        self._structure(mocked_store, monkeypatch)
        transfer_mock = Mock()
    ...
    def test_target_file_trailing_slash_2(self,
                                          mocked_store,
                                          partial_mocked_store,
                                          monkeypatch):
        self._structure(mocked_store, monkeypatch)
    ...
    def test_target_dir_trailing_slash(self,
                                       mocked_store,
                                       partial_mocked_store,
                                       monkeypatch):
        self._structure(mocked_store, monkeypatch)

...
@click.option(
    ...
    default=True,
    is_flag=True,
    help="Reconnect if the link to server is lost, on by default.",
)
@click.option(
    "--no-key-check",
    is_flag=True,
...
async def port_forward(
    ...
    forwarder: Callable[
        [Root, str, int, int, bool], Awaitable[None]
    ] = port_forward_reconnect if reconnect else port_forward_simple
...
async def port_forward_reconnect(
    ...
    while True:
        try:
            ...
            now = time.asctime()
            click.echo(
                f"Reconnect localhost:{local_port} => " f"{job_id}:{job_port} @ {now}"
            )

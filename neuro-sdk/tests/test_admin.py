        ...
        assert clusters == {
            "default": _ConfigCluster(
                ...
                cloud_provider=_CloudProvider(
                    ...
                    storage=_Storage(
                        description="Filestore",
                        instances=[
                            _StorageInstance(size=1024 * 2**20),
                            _StorageInstance(name="org", size=1024 * 2**20),
                        ],
                    ),
                ),
            ),
            "on-prem": _ConfigCluster(
                ...
                cloud_provider=_CloudProvider(
                    ...
                    storage=_Storage(
                        description="NFS",
                        instances=[_StorageInstance(size=1024 * 2**20)],
                    ),
                ),
            ),
...
async def test_update_cluster_resource_presets(
    ...
    async with make_client(srv.make_url("/api/v1")) as client:
        await client._admin.update_cluster_resource_presets(
            ...
            {
                "cpu-micro": Preset(
                    credits_per_hour=Decimal("10"), cpu=0.1, memory=100 * 2**20
                ),
                "cpu-micro-p": Preset(
